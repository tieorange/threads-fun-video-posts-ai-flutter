...
class VideoRemoteDatasource {
  ...
  Map<String, dynamic> _payloadSummary(Map<String, dynamic> aiPayload) {
    ...
    final ids = moments.whereType<Map>().map((m) => m['id']).whereType<String>().take(5).toList();
    return {
  ...
  Future<AnalyzeResponseDto> analyzeVideo(String url, String language) async {
    ...
    try {
      final response = await _dio.post<Map<String, dynamic>>(
        ...
        options: Options(headers: {'x-request-id': requestId, 'x-client-request-id': requestId}),
      );
      ...
      _log.info(
        ...
        data: {'status': response.statusCode, 'serverRequestId': serverRequestId},
      );
      final data = response.data;
      if (data == null) {
        throw Exception('Analyze response body is null');
      }
  ...
  Future<String> submitProcess(String youtubeUrl, Map<String, dynamic> aiPayload) async {
    final start = DateTime.now().millisecondsSinceEpoch;
    final requestId = _newCorrelationId();
    _log.info(
      'http_process_start',
    ...
    try {
      ...
      _log.error(
        ...
        layer: AppLayer.data,
        requestId: requestId,
        data: {..._extractServerError(e), 'serverRequestId': _responseRequestId(e.response)},
      );
      rethrow;
    }
  }
  ...
  Future<JobStatusDto> getJobStatus(String jobId) async {
    ...
    try {
      ...
      final serverRequestId = _responseRequestId(response);
      _log.debug(
        'http_job_status_done',
        'Status: ${dto.status} (${dto.progress}%)',
        layer: AppLayer.data,
        requestId: requestId,
        jobId: jobId,
